...
impl<'a> Parser<'a> {
    ...
    #[track_caller]
    fn visibility(&mut self) -> ParseResult<Visibility> {
    ...
    /// ```
    fn parse_type_decl(&mut self, type_visibility: Option<Visibility>) -> ParseResult<TypeDecl> {
    ...
    fn then(&mut self) -> ParseResult<(Option<Else>, u32)> {
    ...
    fn function_call_arguments(&mut self) -> ParseResult<(Vec<Expr>, u32)> {
        self.eat(TokenType::LeftParen)?;
    ...
    fn function_call(&mut self, input: impl Into<Option<(Sym, u32)>>) -> ParseResult<FunctionCall> {
    ...
    fn assign(&mut self, input: impl Into<Option<(Sym, u32)>>) -> ParseResult<Assign> {
    ...
    fn array(&mut self) -> ParseResult<(Expr, u32)> {
        trace!("Parsing array");
    ...
    fn expr(&mut self) -> ParseResult<(Expr, u32)> {
        trace!("Parsing expr");
    ...
    fn extended_expr(&mut self, expr: Expr, start: u32) -> ParseResult<(Expr, u32)> {
        let next = self.peek()?;
        if [
            TokenType::Plus,
            TokenType::Minus,
    ...
    fn conditional_clause(&mut self) -> ParseResult<(Either<If, Vec<Statement>>, u32)> {
...
impl<'a> Parser<'a> {
    ...
    #[inline]
    fn escape_string(&mut self, string: &str) -> ParseResult<String> {
